...
public class MessageConductor {
    ...
    public void steerMessage(String payload, String topic) throws JSONException {
        ...
        Intent actionIntent = new Intent(Constants.ACTION_INTENT_SUBSCRIBED_MSG_RECEIVED);

        if(topic.contains("temperature")){
            actionIntent.putExtra(Constants.TEMPERATURE, payload);
        } else if(topic.contains("humidity")){
            actionIntent.putExtra(Constants.HUMIDITY, payload);
        } else if(topic.contains("relay")){
            actionIntent.putExtra(Constants.RELAY, payload);
        }

        actionIntent.putExtra(Constants.SUBSCRIDED_MSG, topic + " : " + payload);
